...
Помимо цифр в вышеприведённой таблице, следует обратить внимание на такую особенность снимков Lacmus Drone Dataset, как большой дисбаланс классов: отношение площади фона к площади прямоугольного анкора с искомым объектом составляет несколько тысяч.
...
Анкоры имеют площади от [16 х 16] до [256 x 256] для каждого уровня пирамиды от P3 до P7 соответственно, с шагом смещения (strides) [8 - 128] px. Такой размер позволяет анализировать мелкие объекты и некоторую окрестность вокруг. Например ветка, если не учитывать окружающую ее действительность, очень похожа на лежащего человека.  
...
Такой подход позволяет сократить вычислительные затраты при обучении детектора.
...
В данный момент мы ведём переговоры с компанией [DTL](http://immersiya.com/digital-technology-lab) и надеемся, что сотрудничество сложится.
Уникальность GPU-серверов DTL состоит в применении иммерсионного охлаждения: погружения стоек в специальную диэлектрическую жидкость. Выглядит это вот так:
...
- отсняли и разметили уникальный датасет, последняя версия которого включает в себя более 2000 снимков;
...
- запустить InClass-соревнование на Kaggle для всех желающих прокачать свои DL/CV навыки и поиска наилучших SOTA решений;
...
Мы - OpenSource-проект, и радостно примем всех желающих!

Вот ссылки на наши репозитории на github:

- https://github.com/lacmus-foundation - заглавная страница 
...
Проект разрабатывается разными людьми, с разным опытом и интересами, но объединяет нас одно: нам всем интересно пробовать что-то новое, экспериментировать и создавать продукт, делающий мир лучше. Мы все хотим, чтобы IT-технологии был открытыми, полезными и помогающими человечеству.
